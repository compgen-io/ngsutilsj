...
public class RepeatAnnotate extends AbstractOutputCommand {
    ...
    public void exec() throws NGSUtilsException, IOException {
        ...
        Annotator ann = new RepeatMaskerAnnotator(repeatFilename);
        if (verbose) {
        ...
        for (String line: new StringLineReader(filename)) {
            ...
            List<String[]> annVals = ann.findAnnotation(ref, start, end, strand); 
           
            for (int i=0; i < ann.getAnnotationNames().length; i++) {
                String[] vals = new String[annVals.size()];
                for (int j=0; j < annVals.size(); j++) {
                    vals[j] = annVals.get(j)[i];
                }
