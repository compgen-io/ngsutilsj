...
public class FastqFilter implements NGSExec {
	...
	public void filter() throws IOException, NGSUtilsException {
		...
		List<FilterIterable> iters = new ArrayList<FilterIterable>();
		Iterable<FastqRead> parent = reader;
		
		if (prefixTrimLength > 0) {
			parent = new FilterIterable(new PrefixFilter(parent, verbose, prefixTrimLength));
			iters.add((FilterIterable) parent);	
		}
		
		if (suffixQuality != null) {
			parent = new FilterIterable(new SuffixQualFilter(parent, verbose, suffixQuality.charAt(0)));
			iters.add((FilterIterable) parent);	
		}
		
		if (minimumSize > 0) {
			parent = new FilterIterable(new SizeFilter(parent, verbose, minimumSize));
			iters.add((FilterIterable) parent);
		}

		for (FastqRead read: parent) {
			if (read != null) {
		...
		for (FilterIterable iter:iters) {
			System.err.println(iter.getName()+"\t"+iter.getTotal()+"\t"+iter.getAltered()+"\t"+iter.getRemoved());
		}
		}
	}
