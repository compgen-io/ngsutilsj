...
public class GTFAnnotate extends AbstractOutputCommand {
    ...
    @Option(desc = "Add gene region (intron, exon, etc) annotation", name = "gene-region")
    public void setGenicRegion(boolean val) {
        if (val) {
            outputs.add("gene_region");
        }
    }
            ...
            try {
                ...
                if (regionCol > -1) {
                    ...
                    if (annVals.size() == 0 && junctionWithinGene && strandCol != -1) {
                        ...
                        // this will look for genes that include this
                        // junction-span
                ...
                if (annVals.size() == 0 && genomeSpan != null) {
                    ...
                    GenomeSpan antiSpan = new GenomeSpan(genomeSpan.ref, genomeSpan.start,
                            genomeSpan.end, genomeSpan.strand.getOpposite());
                ...
                for (int i = 0; i < annVals.size(); i++) {
                    ...
                    if (genomeSpan != null) {
                        // determine region annotation based on start/end of the
                        // region
                        GenicRegion start = ann.findGenicRegionForPos(genomeSpan.getStartPos(),
                                gene.getGeneId());
                        GenicRegion end = ann.findGenicRegionForPos(genomeSpan.getEndPos(),
                                gene.getGeneId());

                        if (start == end) {
                            regions[i] = start.toString();
                        } else {
                            if (start.isExon != end.isExon && start != GenicRegion.INTERGENIC
                                    && end != GenicRegion.INTERGENIC) {
                                if (start.isSense) {
                    ...
                    for (String output : outputs) {
                        ...
                        case "gene_name":
                            if (geneNames.length > 0) {
                                ...
                                // output *something* if the gene name isn't
                                // known.
                                writer.write(StringUtils.join(",", geneIds));
                            }
                            break;
